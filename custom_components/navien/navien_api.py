...
class NavilinkConnect():
    ...
    def __init__(self, userId, passwd, device_index = 0, polling_interval = 15, aws_cert_path = certifi.where(), subscribe_all_topics=False):
        """
    ...
    async def _connect_aws_mqtt(self):
        global mqtt
        if mqtt is None:
            import AWSIoTPythonSDK.MQTTLib as mqtt  # type: ignore
    ...
    async def _heating_temperature_command(self, temp, channel_number):
        """Best-effort: set central heating temperature (not all devices support this)."""
        topic = self.topics.control()
        payload = self.messages.heating_temperature(temp, channel_number)
        session_id = self.get_session_id()
        payload["sessionID"] = session_id
        self.response_events[session_id] = asyncio.Event()
        await self.async_publish(topic=topic, payload=payload, session_id=session_id)
...
class NavilinkChannel:
        ...
        if len(self.callbacks) > 0:
            for callback in list(self.callbacks):
                try:
                    ...
                    _LOGGER.debug("Navien callback failed: %s", err, exc_info=True)
    ...
    async def set_heating_temperature(self, temp):
        """Best-effort: set central heating target temperature."""
        if not self.waiting_for_response:
            self.waiting_for_response = True
            await self.hub._heating_temperature_command(temp, self.channel_number)
            self.publish_update()
            self.waiting_for_response = False
    ...
    def convert_channel_status(self,channel_status):
        ...
        if self.channel_info.get("temperatureType",2) == TemperatureType.CELSIUS.value:
            ...
            if channel_status["unitType"] in [
                ...
                # Heating setpoint (some models expose this alongside DHW).
                for _k in ("CHSettingTemp","chSettingTemp","HeatingSettingTemp","heatingSettingTemp","HeatingSetTemp","heatingSetTemp","HeatingTargetTemp","heatingTargetTemp"):
                    if _k in channel_status:
                        channel_status[_k] = round(channel_status[_k] / 2.0, 1)
    ...
    def convert_channel_info(self,channel_info):
        if channel_info.get("temperatureType",2) == TemperatureType.CELSIUS.value:
            ...
            # Heating setpoint ranges (if present).
            for _k in ("setupHeatingTempMin","setupHeatingTempMax","setupCHTempMin","setupCHTempMax","heatingTempMin","heatingTempMax","chTempMin","chTempMax"):
                if _k in channel_info:
                    channel_info[_k] = round(channel_info[_k] / 2.0, 1)
...
class Messages:
    ...
    def heating_temperature(self, temp, channel_number):
        """Set the central heating temperature (best-effort; device support varies)."""
        return {
            "clientID": self.client_id,
            "protocolVersion": 1,
            "request": {
                ...
                "command": 33554435,
                "control": {"channelNumber": channel_number, "mode": "HeatingTemperature", "param": [temp]},
